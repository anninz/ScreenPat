...
/*    ArrayList<ImageView> mRecyledShits = new ArrayList<ImageView>();
    ...
    public RoachShitFactory(IPlugAPI fxService, int resShitID) {
        super(fxService);
        shitBitmap = BitmapFactory.decodeResource(fxService.getResources(), resShitID);
    ...
    public void DoShit(int currentPatX, int currentPatY) {
        ...
        if ((AbsShitProvider.shitCount - mRecyledShits.size()) > MAX_SHIT_NUM) {
...
//        if (mWindowManager == null) return;
        ...
        if (mRecyledShits.size() > 0) {
            ...
            ((IPlugAPI) mContext).updateView(imageView, wmShitParams);
            imageView.setVisibility(View.VISIBLE);
        } else {
            AbsShitProvider.shitCount ++;
...
//            imageView.setImageResource(R.drawable.shit);//0728
            ...
            imageView.setTag(AbsShitProvider.shitCount);
            mShitMap.put(AbsShitProvider.shitCount, imageView);
...
//        mShitLayout = (FrameLayout) inflater.inflate(R.layout.shit_layout1, null);
        initPatParams(wmShitParams);

//        SharedPreferences sp = mContext.getSharedPreferences("data", Context.MODE_PRIVATE);
        int patSize = mContext.getInt("size",45) / 2;
